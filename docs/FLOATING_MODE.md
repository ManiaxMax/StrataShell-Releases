...
September 13 source update: Settings exposes both modes with explicit labels and search. New installations default to 0% background transparency; existing opacity settings are preserved. Super + Arrow focuses visible windows directionally, and Super + \ toggles fullscreen with previous-bound restoration. Rail clicks select their monitor; edge reveal and the command launcher remain available during fullscreen. Secondary widget placement, Snip auto-copy, terminal tabs and AI CLI setup are described in SETTINGS.md. Historical installation evidence below applies only to those earlier releases.
...
Follow-up fixes (source): minimized native caption strips are hidden while their iconic state and dock entries are preserved. Clicking a running dock icon always activates/restores it. STRATA title regions include their blank space as a pointer target for moving in Floating mode, while Tiled mode preserves Center Stage placement and requires `Super + Ctrl` to float or resize. Background dimming waits until opening reveals settle and releases its opacity before other presentation animations capture their baseline, preventing invisible app centers. The desktop signature moves to the top-left in Floating mode. Wallpaper, Window Mode and other OSD notifications appear at the top in Floating mode and at the bottom in Tiled mode, revealing inward from that edge. The default background transparency is now 80%; schema 36 migrates the old 90% default while preserving other customized values.
...
Press **Super + Ctrl + D**, or use **UI & Theme → Interface → Window Management Mode**. The choice persists across launches. Existing profiles remain Tiled after migration.
...
- Window Mode/Widgets becomes Show Widgets / Hide Widgets per monitor. Existing monitor widget eligibility remains respected.
