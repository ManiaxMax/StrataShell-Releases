...
The default owner test package is a prerelease targeting the distribution repository's `preview` branch. It is framework-dependent and intentionally omits the .NET runtime, full Setup executable, wallpaper library, and Stable-only extras. Current development targets .NET 10; a registered .NET 10 Desktop runtime can run its Preview packages. Alternatively, the installer can reuse the exact hash-verified runtime files from a compatible Stable installation. An older .NET 9 Stable runtime cannot satisfy this bridge. A failed runtime/self-test check prevents activation. Only Stable installers and bundles include the complete .NET 10 Desktop runtime, including Windows Forms; the public Preview ZIP remains runtime-free. Published older packages retain their original runtime.
